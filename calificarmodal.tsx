...
const InscripcionesModal: React.FC<InscripcionesModalProps> = ({ onClose }) => {
  ...
  const [inscritoSeleccionado, setInscritoSeleccionado] = useState<{nombre: string, doc:number, idCur: number, Especificacion: string, id: number} | null>(null);
  ...
  const abrirModalCalificar = async (nombre: string, doc: number, idCur: number, Especificacion: string, id: number) => {
    setInscritoSeleccionado({ nombre, doc, idCur, Especificacion, id });
            ...
            {inscripciones.map((insc, index) => (
              ...
              >
                ...
                <span className="text-black">{insc.Especificacion || 'Sin Nota'}</span>
                  ...
                  <button
      onClick={() => abrirModalCalificar(insc.nombre, insc.docInscr, insc.idCur, insc.Especificacion, insc.id )}
